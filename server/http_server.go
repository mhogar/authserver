...
func CreateHTTPServerRunner(DBConnection database.DBConnection, requestHandler controllers.RequestHandler) Runner {
	server := &HTTPServer{
		Server: http.Server{
			Addr:    ":8080",
			Handler: router.CreateRouter(requestHandler),
		},
	}
